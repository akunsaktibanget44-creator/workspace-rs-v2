...
import { AlertTriangle, Clock, TrendingUp, Users2, Activity, Flame, CalendarClock, Loader2 } from "lucide-react";
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs";
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from "@/components/ui/select";
import {
  monitoringDeadline, monitoringWorkload, monitoringCompliance, monitoringStagnant, monitoringDivProgress, listDivisi,
} from "@/lib/api";
      ...
      <div className="rounded-2xl border border-emerald-100 bg-gradient-to-r from-emerald-900 to-emerald-950 p-5 text-white">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-100/80">Cockpit untuk mengontrol deadline, beban kerja, konsistensi amaliyah, dan progres tim.</p>
          </div>
...
function TaskCard({ t, tone = "emerald" }) {
  const tones = {
  ...
  return (
    <div className={`rounded-lg border p-3 ${tones[tone]}`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-bold ${tone === "red" ? "bg-red-600 text-white" : tone === "amber" ? "bg-amber-500 text-white" : "bg-emerald-700 text-white"}`}>{t.deadline}</span>
      </div>
...
function DeadlineRadar({ divisiId }) {
  ...
  useEffect(() => { setData(null); monitoringDeadline(divisiId ? { divisi_id: divisiId } : {}).then(setData).catch(() => setData({ overdue: [], today: [], upcoming: [], summary: {} })); }, [divisiId]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Column title="Overdue" tone="red" items={data.overdue} />
        <Column title="Deadline Hari Ini" tone="amber" items={data.today} />
        <Column title="3 Hari Kedepan" tone="emerald" items={data.upcoming} />
      </div>
    </div>
  );
}
...
function Column({ title, tone, items }) {
  ...
  return (
    <div className={`rounded-xl border ${borders[tone]} bg-white p-3`}>
      <h4 className={`mb-2 text-xs font-semibold uppercase tracking-wider ${heads[tone]}`}>{title} <span className="ml-1 rounded bg-slate-100 px-1.5 text-[10px] text-slate-700">{items.length}</span></h4>
      {items.length === 0 ? <p className="p-6 text-center text-xs text-emerald-800/50 italic">Kosong. Alhamdulillah.</p>
        : <div className="max-h-[420px] space-y-2 overflow-y-auto pr-1">{items.map((t) => <TaskCard key={t.id} t={t} tone={tone} />)}</div>}
    </div>
  );
}
